...
如果真实桌面行为仍异常，先运行 `pnpm qa:desktop:auto` 生成包含当前桌面源码指纹的自动报告，再运行 `pnpm qa:desktop` 单次启动人工验收 `.app`，逐项确认启动时只有宠物、焦点保持、Space／普通全屏可见性、菜单栏恢复、偏好设置重建和受控开发入口。人工命令需要 macOS 交互式桌面并拒绝缺失、无效或源码指纹不匹配的自动报告；自动 smoke 不能替代真实点击与视觉确认。
