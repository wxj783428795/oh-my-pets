...
`qa:desktop:auto` 使用构建后的真实 Tauri 可执行文件检查启动焦点、示例宠物加载、窗口隐藏与托盘处理器恢复、点击穿透开关和诊断导出，并把当前桌面源码指纹写入报告。随后运行的 `qa:desktop` 只会复用指纹与当前工作树一致的已通过报告并单次启动 `.app`，要求验收人在真实 macOS 桌面确认菜单栏点击、视觉显示和点击穿透体验；报告缺失、报告无效、源码不匹配或非交互执行都不能算作人工 QA 通过。
...
在 macOS 的 Tauri 2 透明 WebView 路线上，整窗点击穿透与系统级文件拖放不能同时工作。点击穿透默认关闭；开启后可在同一高级开发预览中关闭。若该窗口已收起，先从菜单栏打开偏好设置，再进入高级开发预览恢复交互。
